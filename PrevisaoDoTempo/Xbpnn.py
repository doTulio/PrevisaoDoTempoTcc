...
def treinamento(nomearquivo):
    ...
    weights.wi = list(weights.wi)
    for i in range(len(weights.wi)):
        weights.wi[i] = list(weights.wi[i])

    weights.wo = list(weights.wo)
    for i in range(len(weights.wo)):
        weights.wo[i] = list(weights.wo[i])
